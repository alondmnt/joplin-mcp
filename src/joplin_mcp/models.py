...
class MCPNote(BaseModel):
    ...
    updated_time: int = Field(..., description="Last update timestamp (Unix milliseconds)")
...
class MCPNotebook(BaseModel):
    ...
    master_key_id: Optional[str] = Field(None, description="Master key ID for encryption")
...
class MCPSearchResult(BaseModel):
    ...
    items: List[Dict[str, Any]] = Field(default_factory=list, description="Search result items")
...
class MCPPaginatedResponse(BaseModel):
    ...
    items: List[Dict[str, Any]] = Field(default_factory=list, description="Paginated items")
    ...
    endpoint: Optional[str] = Field(None, description="API endpoint for navigation links")
...
class MCPErrorResponse(BaseModel):
    ...
    timestamp: Optional[int] = Field(None, description="Error timestamp (Unix milliseconds)")
    ...
    def get_field_errors(self, field_name: str) -> List[str]:
        ...
        return [
            error["message"] for error in self.validation_errors
            if error.get("field") == field_name
        ]
    
    @classmethod
    def from_exception(cls, exception: Exception, error_code: str, status_code: int = 500) -> "MCPErrorResponse":
...
class MCPAPIResponse(BaseModel):
    ...
    data: Optional[Dict[str, Any]] = Field(None, description="Response data for successful operations")
    error_info: Optional[MCPErrorResponse] = Field(None, description="Error information for failed operations")
    metadata: Optional[Dict[str, Any]] = Field(None, description="Additional response metadata")
    timestamp: int = Field(default_factory=JoplinTimestamp.now, description="Response timestamp")
    ...
    @classmethod
    def create_success(cls, data: Optional[Dict[str, Any]] = None, metadata: Optional[Dict[str, Any]] = None) -> "MCPAPIResponse":
    ...
    @classmethod
    def create_error(cls, error_code: str, message: str, status_code: int, 
                    details: Optional[Dict[str, Any]] = None,
                    metadata: Optional[Dict[str, Any]] = None) -> "MCPAPIResponse":
    ...
    @classmethod
    def success_response(cls, data: Optional[Dict[str, Any]] = None, metadata: Optional[Dict[str, Any]] = None) -> "MCPAPIResponse":
        """Create a successful API response (alias for create_success)."""
        return cls.create_success(data, metadata)
    
    @classmethod
    def error_response(cls, error_code: str, message: str, status_code: int, 
                      details: Optional[Dict[str, Any]] = None,
                      metadata: Optional[Dict[str, Any]] = None) -> "MCPAPIResponse":
