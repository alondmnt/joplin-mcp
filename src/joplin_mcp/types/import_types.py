...
class ImportOptions:
    ...
    def __post_init__(self):
        ...
        if self.handle_duplicates not in valid_duplicate_strategies:
            raise ValueError(f"handle_duplicates must be one of {valid_duplicate_strategies}")
            
        valid_attachment_strategies = ["link", "embed", "skip"]  
        if self.attachment_handling not in valid_attachment_strategies:
            raise ValueError(f"attachment_handling must be one of {valid_attachment_strategies}")
