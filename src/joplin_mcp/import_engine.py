...
class JoplinImportEngine:
    ...
    async def create_note_safe(
        ...
        try:
            ...
            notebook_id = None
            if note.notebook:
                notebook_id = await self.ensure_notebook_exists(note.notebook, options, result)
            elif options.target_notebook:
                notebook_id = await self.ensure_notebook_exists(options.target_notebook, options, result)
    ...
    async def ensure_notebook_exists(
        ...
        try:
            ...
            result.add_warning(f"Could not create/find notebook '{notebook_name}': {str(e)}")
        ...
        try:
            ...
            for tag_id in tag_ids:
                try:
                    ...
                    logger.warning(f"Failed to apply tag {tag_id} to note {note_id}: {e}")
    ...
    async def _find_existing_note(self, title: str, notebook_id: Optional[str]) -> Optional[str]:
    ...
    async def _generate_unique_title(self, base_title: str, notebook_id: Optional[str]) -> str:
