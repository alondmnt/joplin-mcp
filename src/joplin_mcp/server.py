...
try:
    ...
    ToolCapabilities = MagicMock()
    PromptCapabilities = MagicMock()
    ResourceCapabilities = MagicMock()
    MCP_AVAILABLE = False
...
class JoplinMCPServer:
    ...
    def __init__(
        ...
        try:
            ...
            if hasattr(self.client, '_mock_name') or str(type(self.client)).find('Mock') != -1:
            ...
            if not (hasattr(self.client, '_mock_name') or str(type(self.client)).find('Mock') != -1):
                raise Exception(f"Joplin connection validation failed: {e}")
        
        # Create underlying MCP server
        self._mcp_server = Server(self.server_name)
    ...
    def validate_tool_input(self, tool_name: str, input_data: Dict[str, Any]) -> tuple[bool, List[str]]:
        ...
        for param, value in input_data.items():
            ...
            if expected_type == 'string':
                ...
                # Check enum values
                    ...
                    errors.append(f"Parameter '{param}' must be one of: {prop_schema['enum']}")
                ...
                if not isinstance(value, int):
                    ...
                    if 'minimum' in prop_schema and value < prop_schema['minimum']:
                        errors.append(f"Parameter '{param}' must be at least {prop_schema['minimum']}")
                    if 'maximum' in prop_schema and value > prop_schema['maximum']:
                        errors.append(f"Parameter '{param}' must be at most {prop_schema['maximum']}")
        ...
        try:
            ...
            if not results:
                return {
                    'content': [{
                        'type': 'text',
                        'text': f'No notes found for query: "{query}"'
                    }]
                }
    ...
    def _format_single_search_result(self, note: Dict[str, Any]) -> str:
        ...
        if created_time:
            try:
                import datetime
                created_date = datetime.datetime.fromtimestamp(created_time / 1000).strftime('%Y-%m-%d %H:%M')
        ...
        if updated_time:
            try:
                import datetime
                updated_date = datetime.datetime.fromtimestamp(updated_time / 1000).strftime('%Y-%m-%d %H:%M')
    ...
    def _format_note_details(self, note: Dict[str, Any], include_body: bool = True) -> str:
        ...
        created_time = note.get('created_time')
        updated_time = note.get('updated_time')
        parent_id = str(note.get('parent_id', '')).strip() if note.get('parent_id') else ''
    ...
    def _build_note_metadata(
        ...
        if tags and isinstance(tags, (list, tuple)):
            ...
            if valid_tags:
                tags_str = ', '.join(valid_tags[:10])  # Limit to 10 tags for readability
    ...
    def _validate_create_note_params(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        parent_id = params.get('parent_id', '').strip() if params.get('parent_id') else ''
        ...
        validated['is_todo'] = bool(is_todo) if not isinstance(is_todo, bool) else is_todo
        
        todo_completed = params.get('todo_completed', False)
        validated['todo_completed'] = bool(todo_completed) if not isinstance(todo_completed, bool) else todo_completed
    ...
    async def handle_delete_note(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return self._build_operation_response(
                ...
                entity_type="note",
                entity_id=note_id,
                details="The note has been permanently removed from Joplin." if success else None
            )
    ...
    async def handle_list_tags(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return self._build_list_response(
                items=tags,
                entity_type="tags",
                formatter_method=self._format_tags_list
            )
    ...
    def _validate_required_id_parameter(self, params: Dict[str, Any], param_name: str) -> str:
    ...
    def _validate_dual_id_parameters(self, params: Dict[str, Any], param1_name: str, param2_name: str) -> tuple[str, str]:
    ...
    def _build_operation_response(self, success: bool, operation: str, entity_type: str, entity_id: str, details: str = None) -> Dict[str, Any]:
        ...
        if success:
            emoji = "✅"
            status = "Successfully"
            action_details = details or f"The {entity_type} has been successfully {operation} in Joplin."
        else:
            emoji = "❌"
            status = "Failed to"
            action_details = details or f"The {entity_type} {operation} operation was not successful."
        
        message = f"{emoji} {status} {operation} {entity_type}"
        formatted_details = f"**{entity_type.title()} ID:** {entity_id}\n\n{action_details}"
        formatted_text = f"{message}\n\n{formatted_details}"
    ...
    def _build_creation_response(self, entity_type: str, title: str, entity_id: str, additional_info: str = None) -> Dict[str, Any]:
    ...
    def _validate_update_note_params(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'body' in params:
            update_params['body'] = str(params['body']) if params['body'] is not None else ''
    ...
    def _build_list_response(self, items: List[Dict[str, Any]], entity_type: str, formatter_method) -> Dict[str, Any]:
