...
from typing import Optional, Dict, Any, Union, List, Generator, Callable
    ...
    def get_average_response_time(self) -> float:
        ...
        with self._lock:
            if not self.performance_history:
                return 0.0
            return sum(self.performance_history) / len(self.performance_history) * 1000  # Convert to ms
...
class JoplinMCPClient:
    ...
    def __init__(
        ...
        try:
            ...
            if config is None and all(param is None for param in [host, port, token, timeout, verify_ssl]):
            ...
            self._joppy_client = ClientApi(
                token=config.token,
                url=config.base_url
            )
            
            # Initialize connection management components
            self._connection_pool = ConnectionPool(max_connections=1)  # Single connection for now
            ...
            raise JoplinClientError("joppy library is required but not installed") from e
        except Exception as e:
    ...
    def ping(self) -> bool:
        ...
        try:
            ...
            if success:
                self._connection_metrics.add_event('connection_success', 'Ping successful')
    ...
    def ping_with_timeout(self, timeout: int = 5) -> bool:
        ...
        try:
            ...
            if "timeout" in str(e).lower():
                raise JoplinClientError(f"Connection timeout after {timeout}s: {e}") from e
            raise JoplinClientError(f"Ping failed: {e}") from e
    ...
    def check_network_connectivity(self) -> Dict[str, Any]:
        ...
        try:
            ...
            with socket.create_connection((self.config.host, self.config.port), timeout=5):
    ...
    def ping_cached(self, cache_ttl: int = 60) -> bool:
        ...
        if (self._ping_cache['result'] is not None and 
            (current_time - self._ping_cache['timestamp']) < cache_ttl):
    ...
    def _safe_get_attr(self, obj, attr_name: str, default: Any, converter: Optional[Callable] = None) -> Any:
    ...
    def transform_note_to_mcp(self, joppy_note) -> MCPNote:
        ...
        try:
            return MCPNote(
                ...
                is_todo=self._safe_get_attr(joppy_note, 'is_todo', False, self._safe_bool_converter),
                todo_completed=self._safe_get_attr(joppy_note, 'todo_completed', False, self._safe_bool_converter),
                is_conflict=self._safe_get_attr(joppy_note, 'is_conflict', False, self._safe_bool_converter),
                ...
                markup_language=self._safe_get_attr(joppy_note, 'markup_language', 1, int)
            )
        except Exception as e:
    ...
    def transform_notebook_to_mcp(self, joppy_notebook) -> MCPNotebook:
        ...
        try:
            return MCPNotebook(
                ...
                created_time=self._safe_get_attr(joppy_notebook, 'created_time', 0, int),
                updated_time=self._safe_get_attr(joppy_notebook, 'updated_time', 0, int),
    ...
    def transform_search_results_to_mcp(
        ...
        try:
            ...
            raise JoplinClientError(f"Failed to transform search results to MCP: {e}") from e
    ...
    def transform_notes_to_mcp_batch(self, joppy_notes: List) -> List[MCPNote]:
        ...
        try:
            ...
            raise JoplinClientError(f"Failed to transform notes batch to MCP: {e}") from e
    ...
    def enhanced_search(
        ...
        try:
            ...
            if fuzzy_matching:
                raw_results = self._apply_fuzzy_matching(raw_results, query, fuzzy_threshold)
            
            # Apply filters
            filtered_results = self._apply_enhanced_filters(raw_results, query, **(filters or {}))
            
            # Apply sorting
            sorted_results = self._apply_sorting(filtered_results, sort_by, sort_order, boost_fields)
            ...
            if aggregations:
                result.aggregations = self._apply_aggregations(filtered_results, aggregations)
            ...
            if format == "json":
                ...
                if include_metadata:
                    export_data['metadata'] = {
                        'search_metadata': getattr(search_result, 'search_metadata', {}),
    ...
    def _apply_enhanced_filters(self, results: List[Any], query: str, **kwargs) -> List[Any]:
        ...
        try:
            ...
            if date_field:
                ...
                results = self._apply_date_range_query(results, date_field, start_date, end_date)
    ...
    def _apply_sorting(self, results: List[Any], sort_by: str, sort_order: str, boost_fields: Optional[Dict[str, float]]) -> List[Any]:
        ...
        try:
            ...
            if sort_by == 'relevance':
                ...
                sorted_results.sort(
                    key=lambda x: getattr(x, '_boost_score', 1.0),
                    reverse=reverse
                )
            else:
    ...
    def _add_highlighting(self, item_dict: Dict[str, Any], query: str, highlight_tags: tuple) -> Dict[str, Any]:
    ...
    def _calculate_relevance_score(self, note: MCPNote, query: str, boost_fields: Optional[Dict[str, float]]) -> float:
    ...
    def _generate_facets(self, results: List[Any], facet_fields: List[str]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    def _get_related_content(self, query: str, limit: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            recent_searches = []
            for cache_key, (result, _) in self._search_cache.items():
    ...
    def _apply_fuzzy_matching(self, results: List[Any], query: str, threshold: float) -> List[Any]:
        ...
        try:
            ...
            # Sort by fuzzy score
            matched_results.sort(key=lambda x: getattr(x, '_fuzzy_score', 0.0), reverse=True)
        ...
        try:
            ...
            for match in date_range_matches:
                ...
                try:
                    ...
                    raise JoplinClientError(f"Invalid date format in query: {start_date} or {end_date}")
    ...
    def _apply_aggregations(self, results: List[Any], aggregations: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if kwargs.get('fuzzy_matching'):
                raw_results = self._apply_fuzzy_matching(
                    raw_results,
                    query,
                    kwargs.get('fuzzy_threshold', 0.8)
                )
            
            if kwargs.get('filters'):
                raw_results = self._apply_enhanced_filters(raw_results, query, **kwargs['filters'])
            ...
            for batch in self._stream_search_results(sorted_results, batch_size, transform_item):
                result = MCPSearchResult(
                    items=batch,
                ...
                result.search_metadata = {
                    ...
                    'fuzzy_threshold': kwargs.get('fuzzy_threshold') if kwargs.get('fuzzy_matching') else None,
                ...
                if kwargs.get('return_pagination_info'):
                    result.pagination = {
                        ...
                        'offset': offset,
                        'total_count': total_count,
                        'total_pages': (total_count + batch_size - 1) // batch_size if batch_size > 0 else 1,
                        'has_more': result.has_more
                    }
                
                if kwargs.get('include_facets') and kwargs.get('facet_fields'):
                    result.facets = self._generate_facets(sorted_results, kwargs['facet_fields'])
                
                if kwargs.get('include_related'):
                    result.suggestions = self._get_related_content(
                        query,
                        kwargs.get('related_limit', 3)
                    )
            ...
            raise JoplinClientError(f"Failed to stream enhanced search results: {e}") from e
    ...
    def _apply_date_range_query(self, results: List[Any], field: str, start_date: Optional[datetime] = None, end_date: Optional[datetime] = None) -> List[Any]:
    ...
    def _apply_field_query(self, results: List[Any], field: str, value: Any, operator: str = '=') -> List[Any]:
    ...
    def create_note(
        ...
        **kwargs
        ...
        try:
            ...
            if len(parent_id.strip()) < 8 or not re.match(r'^[a-zA-Z0-9]+$', parent_id.strip()):
    ...
    def update_note(
        ...
        **kwargs
        ...
        try:
            ...
            if not update_data:
                raise JoplinClientError("At least one field must be provided for update")
    ...
    def get_all_notebooks(self) -> List[MCPNotebook]:
        ...
        try:
            ...
            # Transform to MCP format
            return [self.transform_notebook_to_mcp(notebook) for notebook in joppy_notebooks]
    ...
    def get_notebooks_with_hierarchy(self) -> List[MCPNotebook]:
        ...
        try:
            ...
            if isinstance(e, JoplinClientError):
                raise
            raise JoplinClientError(f"Failed to get notebooks with hierarchy: {e}") from e

    def create_notebook(self, title: str, parent_id: Optional[str] = None, **kwargs) -> str:
    ...
    def update_notebook(self, notebook_id: str, title: Optional[str] = None, parent_id: Optional[str] = None, **kwargs) -> bool:
    ...
    def delete_notebook(self, notebook_id: str, force: bool = False) -> bool:
        ...
        try:
            ...
            if not force:
                ...
                has_children = any(nb.parent_id == notebook_id.strip() for nb in all_notebooks)
                if has_children:
                    raise JoplinClientError("Cannot delete notebook with children. Use force=True to override.")
    ...
    def delete_notebooks_bulk(self, notebook_ids: List[str]) -> Dict[str, bool]:
        ...
        try:
            ...
            for notebook_id in notebook_ids:
                try:
                    success = self.delete_notebook(notebook_id, force=True)  # Force for bulk operations
            ...
            try:
                ...
                for tag in existing_tags:
                    if tag.title.lower() == normalized_title:
                        return tag.id  # Return existing tag ID instead of creating duplicate
            except JoplinClientError:
    ...
    def search_tags(self, query: str) -> List[MCPTag]:
        ...
        try:
            ...
            matching_tags = [
                ...
                if query_lower in tag.title.lower()
            ]
