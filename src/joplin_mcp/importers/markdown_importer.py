...
class MarkdownImporter(BaseImporter):
    ...
    async def validate(self, source: str) -> bool:
        ...
        if path.is_file():
            ...
            self.validate_file_size(source, self.options.import_options.get("max_file_size_mb", 100))
            return True
            ...
            if not files:
                raise ImportValidationError(f"No supported markdown files found in: {source}")
            ...
            if total_size > max_total_size:
                raise ImportValidationError(f"Total directory size too large: {total_size:.1f}MB (max: {max_total_size}MB)")
    ...
    async def _parse_markdown_file(self, file_path: str, base_path: str) -> Optional[ImportedNote]:
        ...
        try:
            ...
            # Extract metadata
            is_todo = frontmatter.get('todo', False) or frontmatter.get('is_todo', False)
            todo_completed = frontmatter.get('completed', False) or frontmatter.get('todo_completed', False)
            
            # Extract timestamps
            created_time = self._parse_timestamp(frontmatter.get('created', frontmatter.get('date')))
            updated_time = self._parse_timestamp(frontmatter.get('updated', frontmatter.get('modified')))
            ...
            raise ImportProcessingError(f"Failed to parse markdown file {file_path}: {str(e)}")
    ...
    def _extract_title(self, frontmatter: Dict[str, Any], body: str, filename_fallback: str) -> str:
