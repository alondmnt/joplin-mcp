...
from .base import BaseImporter, ImportValidationError, ImportProcessingError
...
class JEXImporter(BaseImporter):
    ...
    async def validate(self, source: str) -> bool:
        ...
        path = Path(source)
        if not path.is_file():
            raise ImportValidationError("JEX importer only supports files, not directories")
            
        if not self.supports_file(source):
            raise ImportValidationError(f"File extension not supported: {path.suffix}")
            
        # Validate file size
        self.validate_file_size(source, self.options.import_options.get("max_file_size_mb", 500))
        ...
        try:
            with tarfile.open(source, 'r') as tar:
                ...
                if not (has_md_files or has_json_files):
                    raise ImportValidationError("JEX file does not contain expected .md or .json files")
                    
        except tarfile.TarError as e:
            raise ImportValidationError(f"Invalid JEX file (not a valid TAR archive): {str(e)}")
    ...
    async def _load_notebooks_metadata(self, extract_path: Path) -> Dict[str, Dict[str, Any]]:
        ...
        for json_file in extract_path.rglob("*.json"):
            try:
                ...
                if isinstance(data, dict) and data.get('type_') == 2:  # Type 2 = Notebook in Joplin
    ...
    async def _load_tags_metadata(self, extract_path: Path) -> Dict[str, Dict[str, Any]]:
        ...
        for json_file in extract_path.rglob("*.json"):
            try:
                ...
                if isinstance(data, dict) and data.get('type_') == 5:  # Type 5 = Tag in Joplin
    ...
    async def _load_resources_metadata(self, extract_path: Path) -> Dict[str, Dict[str, Any]]:
        ...
        for json_file in extract_path.rglob("*.json"):
            try:
                ...
                if isinstance(data, dict) and data.get('type_') == 4:  # Type 4 = Resource in Joplin
        ...
        for json_file in extract_path.rglob("*.json"):
            try:
                ...
                if (isinstance(data, dict) and 
                    data.get('type_') == 6 and 
                    data.get('note_id') == note_id):
        ...
        if attachment_handling == "skip":
            ...
            def replace_resource(match):
                ...
                if resource_id in resources_map:
                    resource_name = resources_map[resource_id].get('title', f'attachment_{resource_id}')
