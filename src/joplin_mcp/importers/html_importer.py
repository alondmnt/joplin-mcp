...
from ..types.import_types import ImportedNote, ImportOptions, ImportValidationError, ImportProcessingError
...
class HTMLImporter(BaseImporter):
    ...
    async def validate(self, file_path: str) -> None:
        ...
        if path.suffix.lower() not in self.get_supported_extensions():
            raise ImportValidationError(f"Unsupported HTML file extension: {path.suffix}")
        ...
        if content is None:
            raise ImportValidationError(f"Could not read HTML file with any supported encoding: {file_path}")
        
        # Basic HTML validation - check for HTML structure
        if not any(tag in content.lower() for tag in ['<html', '<head', '<body', '<div', '<p', '<h1', '<h2']):
            logger.warning(f"File may not be valid HTML (no common HTML tags found): {file_path}")
    ...
    async def _extract_note_data(self, soup: BeautifulSoup, path: Path) -> dict:
        ...
        note_data['html_title'] = title_tag.string.strip() if title_tag and title_tag.string else None
    ...
    async def _convert_to_markdown(self, soup: BeautifulSoup, note_data: dict) -> str:
        ...
        try:
            ...
            markdown_content = markdownify.markdownify(
                ...
                convert=['p', 'div', 'br', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6',
