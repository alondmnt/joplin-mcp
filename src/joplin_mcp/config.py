        ...
        if strict:
            ...
            if value_lower in ('true', '1'):
                ...
                if value_lower in ('y', 'yes', 'on', 'enable', 'enabled'):
                    ...
                    suggestions.append("Use 'true'/'false' or '1'/'0' for boolean values")
    ...
    def parse_int(value: str, field_name: str, strict: bool = False) -> int:
        ...
        try:
            if strict:
                ...
                if '.' in value:
                    raise ConfigError(f"Invalid integer value for {field_name}: '{value}'. Remove decimal point - use whole numbers only")
                
                if value.endswith(('s', 'sec', 'seconds', 'ms', 'milliseconds')):
                    clean_value = value.rstrip('smilecon')
                    if clean_value.isdigit():
                        raise ConfigError(f"Invalid integer value for {field_name}: '{value}'. Use numeric value only (e.g., '{clean_value}') - seconds are assumed")
            
            return int(value)
        except ValueError:
            if strict:
                raise ConfigError(f"Invalid integer value for {field_name}: '{value}'. Use a numeric value (e.g., '30', '8080')")
            else:
...
class ConfigValidator:
    ...
    def validate_host_format(host: str) -> None:
        ...
        if host.startswith(('http://', 'https://')):
            raise ConfigError(f"Host should not include protocol, got '{host}'. Use host name only (e.g., 'localhost')")
        
        if '@' in host:
            raise ConfigError(f"Host should not include username, got '{host}'. Use host name only")
        
        if ':' in host and not ConfigValidator._is_valid_ipv6(host):
            # Check if it looks like host:port
            parts = host.split(':')
            if len(parts) == 2 and parts[1].isdigit():
                raise ConfigError(f"Host should not include port, got '{host}'. Use the 'port' configuration separately")
            else:
                raise ConfigError(f"Invalid host format, got '{host}'. Use a valid hostname or IP address")
    ...
    def validate_token_format(token: Optional[str]) -> None:
        ...
        if len(token) < 10:
            raise ConfigError(f"Token appears to be too short ({len(token)} characters). Expected at least 10 characters")
        
        # Check for obviously invalid characters that might indicate encoding issues
        if any(c in token for c in ['$', '%', '^', '&', '*', '(', ')', ' ']):
            raise ConfigError(f"Token contains invalid characters. Ensure it's properly copied without spaces or special characters")
...
class JoplinMCPConfig:
        ...
        try:
            ...
            if file_path.suffix.lower() == '.json':
                try:
                    ...
                    raise ConfigError(f"Invalid JSON in file {file_path}: {e}. Please check syntax and fix any formatting errors.")
            elif file_path.suffix.lower() in ('.yaml', '.yml'):
                try:
                    data = yaml.safe_load(content)
                except yaml.YAMLError as e:
                    raise ConfigError(f"Invalid YAML in file {file_path}: {e}. Please check syntax and fix any formatting errors.")
            else:
                raise ConfigError(f"Unsupported file format '{file_path.suffix}' for file {file_path}. Use .json, .yaml, or .yml files.")
            
            # Validate data structure
            if not isinstance(data, dict):
                raise ConfigError(f"Configuration file {file_path} must contain a dictionary/object, got {type(data)}. Check file format.")
    ...
    def _validate_file_data(cls, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'host' in data:
            if data['host'] is None:
                ...
                raise ConfigError(f"Invalid data type for 'host': expected string, got {type(data['host'])}")
            else:
        ...
        if 'port' in data:
            if data['port'] is None:
                ...
                raise ConfigError(f"Invalid data type for 'port': expected integer, got {type(data['port'])}")
        ...
        if 'token' in data:
            if data['token'] is None or isinstance(data['token'], str):
                ...
                raise ConfigError(f"Invalid data type for 'token': expected string, got {type(data['token'])}")
        ...
        if 'timeout' in data:
            if data['timeout'] is None:
                ...
                raise ConfigError(f"Invalid data type for 'timeout': expected integer, got {type(data['timeout'])}")
        ...
        if 'verify_ssl' in data:
            if data['verify_ssl'] is None:
                ...
                raise ConfigError(f"Invalid data type for 'verify_ssl': expected boolean, got {type(data['verify_ssl'])}")
    ...
    def from_file_and_environment(
        ...
        **overrides
        ...
        def get_value(key: str, override_key: str, env_value: Any, file_value: Any, default_value: Any):
        ...
        merged_data = {
            'host': get_value('host', 'host_override', env_config.host, config.host, 'localhost'),
            'port': get_value('port', 'port_override', env_config.port, config.port, 41184),
            'token': get_value('token', 'token_override', env_config.token, config.token, None),
            'timeout': get_value('timeout', 'timeout_override', env_config.timeout, config.timeout, 60),
            'verify_ssl': get_value('verify_ssl', 'verify_ssl_override', env_config.verify_ssl, config.verify_ssl, True),
        }
    ...
    @classmethod
    def auto_discover(cls, search_filenames: Optional[List[str]] = None) -> "JoplinMCPConfig":
    ...
    def get_validation_errors(self) -> List[ConfigError]:
        ...
        if not (1 <= self.port <= 65535):
            errors.append(ConfigError(f"Port must be between 1 and 65535, got {self.port}"))
    ...
    @classmethod
    def from_environment_with_suggestions(cls, prefix: str = "JOPLIN_") -> "JoplinMCPConfig":
        """Load configuration from environment with autocorrection suggestions."""
        # Load values from environment with strict parsing
        host = ConfigParser.get_env_var("HOST", prefix) or "localhost"
        
        port_str = ConfigParser.get_env_var("PORT", prefix)
        port = ConfigParser.parse_int(port_str, "port", strict=True) if port_str else 41184
        
        token = ConfigParser.get_env_var("TOKEN", prefix)
        
        timeout_str = ConfigParser.get_env_var("TIMEOUT", prefix)
        timeout = ConfigParser.parse_int(timeout_str, "timeout", strict=True) if timeout_str else 60
        
        verify_ssl_str = ConfigParser.get_env_var("VERIFY_SSL", prefix)
        verify_ssl = ConfigParser.parse_bool(verify_ssl_str, strict=True) if verify_ssl_str else True
    ...
    @classmethod
    def from_environment_with_warnings(cls, prefix: str = "JOPLIN_", warning_collector: Optional[List[str]] = None) -> "JoplinMCPConfig":
    ...
    @classmethod
    def load(cls, config_file: Optional[Union[str, Path]] = None, **overrides) -> "JoplinMCPConfig":
    ...
    def save_to_file(self, file_path: Union[str, Path], format: str = 'auto') -> None:
        ...
        if format == 'auto':
            if file_path.suffix.lower() == '.json':
                ...
                raise ConfigError(f"Cannot auto-detect format for {file_path}. Use explicit format parameter.")
