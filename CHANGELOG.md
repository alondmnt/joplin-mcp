...
Four changes that cut what every call costs, prompted by BennyTramell's report on token use (#59) and tracked in #66. Measured against a 41-notebook instance: `list_notebooks` 8442 to 4990 characters despite gaining `note_count`, `list_tags` 613 to 389, and 600 characters off a 20-result listing from the content-stats line alone.
...
- **Troubleshooting for `cannot import name 'FastMCP' from 'fastmcp'`.** After an in-place upgrade, fastmcp 3.x can be left importable only as an empty namespace package: it ships as two pip distributions (`fastmcp` and `fastmcp-slim`) that share one package directory, and pip's uninstall step can delete shared files, `fastmcp/__init__.py` among them, after the new ones are written. Reinstalling `joplin-mcp` does not help, because the corruption is in the dependency. The troubleshooting guide now has an entry keyed on that error string with the recovery command, force-reinstalling both distributions pinned to the same version (#55, reported by sinewave-2097).
