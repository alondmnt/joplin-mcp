...
class TestMCPNote:
    ...
    def test_note_to_joplin_dict(self, sample_note_data):
        ...
        expected_fields = {
            ...
            "markup_language", "is_todo", "todo_completed", "is_conflict"
        }
...
class TestMCPPaginatedResponse:
    ...
    def test_paginated_response_validation(self):
        ...
        with pytest.raises(ValidationError):
            MCPPaginatedResponse(page=0, per_page=10, total_count=0, total_pages=0)  # page < 1
            
        with pytest.raises(ValidationError):
            MCPPaginatedResponse(page=1, per_page=0, total_count=0, total_pages=0)  # per_page < 1
            
        with pytest.raises(ValidationError):
            MCPPaginatedResponse(page=1, per_page=10, total_count=-1, total_pages=0)  # negative count
...
class TestMCPAPIResponse:
    ...
    def test_api_response_validation(self):
        ...
        with pytest.raises(ValidationError):
            MCPAPIResponse(
                success=True,
                data={"test": "data"},
                error_info=MCPErrorResponse(error_code="TEST", message="Test", status_code=400)
            )
    ...
    def test_api_response_factory_methods(self):
        ...
        # Error response
        error_response = MCPAPIResponse.create_error("NOT_FOUND", "Resource not found", 404)
    ...
    def test_api_response_to_mcp_format(self):
        ...
        # Error response
        error_response = MCPAPIResponse.error_response("FAILED", "Operation failed", 500)
