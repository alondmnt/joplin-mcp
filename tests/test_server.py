...
class TestJoplinMCPServerTools:
    ...
    def test_ping_joplin_tool_exists(self, mock_server):
        ...
        assert ("ping" in ping_tool.description.lower() or 
                "connection" in ping_tool.description.lower() or
                "test" in ping_tool.description.lower())
...
class TestJoplinMCPServerPrompts:
    ...
    def test_note_template_prompt_is_defined(self, mock_server):
        ...
        assert "template" in template_prompt.description.lower() or "note" in template_prompt.description.lower()
    ...
    def test_tag_organization_prompt_is_defined(self, mock_server):
        ...
        assert "tag" in tag_prompt.description.lower() or "organize" in tag_prompt.description.lower()
...
class TestJoplinMCPServerResources:
    ...
    def test_server_info_resource_is_defined(self, mock_server):
        ...
        server_resource = next((r for r in resources if r.uri == "joplin://server_info"), None)
        
        assert server_resource is not None
        assert server_resource.uri == "joplin://server_info"
        assert "server" in server_resource.name.lower() or "info" in server_resource.name.lower()
    
    def test_notebooks_resource_is_defined(self, mock_server):
        """Test that notebooks resource is defined."""
        # This test should fail - resources don't exist yet
        resources = mock_server.get_available_resources()
        notebooks_resource = next((r for r in resources if r.uri == "joplin://notebooks"), None)
    ...
    def test_statistics_resource_is_defined(self, mock_server):
        ...
        # This test should fail - resources don't exist yet
        resources = mock_server.get_available_resources()
        stats_resource = next((r for r in resources if r.uri == "joplin://statistics"), None)
        
        assert stats_resource is not None
        assert stats_resource.uri == "joplin://statistics"
        assert "statistic" in stats_resource.name.lower() or "stats" in stats_resource.name.lower()
...
class TestJoplinMCPServerSearchNotesTool:
    ...
    def test_search_notes_tool_is_registered(self, server):
        ...
        search_tool = next((tool for tool in tools if tool.name == 'search_notes'), None)
        assert search_tool is not None
    ...
    def test_search_notes_tool_validates_input_correctly(self, server):
        ...
        is_valid, errors = server.validate_tool_input('search_notes', invalid_type_input)
        assert is_valid is False
        assert len(errors) > 0
    ...
    @pytest.mark.asyncio
    async def test_search_notes_tool_handler_with_basic_query(self, server, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_search_notes_tool_handler_with_advanced_parameters(self, server, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_search_notes_tool_handler_formats_results_correctly(self, server, mock_client):
    ...
    @pytest.mark.asyncio 
    async def test_search_notes_tool_handler_handles_empty_results(self, server, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_search_notes_tool_handler_handles_client_errors(self, server, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_search_notes_tool_handler_respects_limit_parameter(self, server, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_search_notes_tool_supports_text_search_operators(self, server, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_search_notes_tool_handles_unicode_and_special_characters(self, server, mock_client):
    ...
    def test_search_notes_tool_schema_validation_comprehensive(self, server):
        ...
        test_cases = [
            ...
            ({'query': 'test', 'sort_by': 'invalid_field'}, False),  # Invalid sort field
            ({'query': 'test', 'sort_order': 'invalid_order'}, False),  # Invalid sort order
        ]
        
        for test_input, expected_valid in test_cases:
            is_valid, errors = server.validate_tool_input('search_notes', test_input)
            assert is_valid == expected_valid, f"Failed for input {test_input}: {errors}"
...
class TestJoplinMCPServerGetNoteTool:
    ...
    @pytest.mark.asyncio
    async def test_get_note_tool_handler_with_include_body_parameter(self, server, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_get_note_tool_handler_formats_result_correctly(self, server, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_get_note_tool_handler_handles_note_not_found(self, server, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_get_note_tool_handler_handles_client_errors(self, server, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_get_note_tool_handler_validates_note_id_format(self, server, mock_client):
...
class TestJoplinMCPServerCreateNoteTool:
    ...
    def test_create_note_tool_handler_exists(self, server):
        ...
        assert callable(getattr(server, 'handle_create_note'))
    ...
    @pytest.mark.asyncio
    async def test_create_note_tool_handler_with_minimal_data(self, server, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_create_note_tool_handler_with_todo_parameters(self, server, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_create_note_tool_handler_handles_client_errors(self, server, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_create_note_tool_formats_response_correctly(self, server, mock_client):
    ...
    def test_create_note_tool_schema_validation_comprehensive(self, server):
        ...
        expected_properties = ['title', 'body', 'parent_id', 'is_todo', 'todo_completed', 'tags']
...
class TestJoplinMCPServerUpdateNoteTool:
    ...
    async def test_update_note_tool_handler_exists(self, server):
        ...
        assert callable(getattr(server, 'handle_update_note'))
    ...
    @pytest.mark.asyncio
    async def test_update_note_tool_handler_validates_required_fields(self, server, mock_client):
...
class TestJoplinMCPServerDeleteNoteTool:
    ...
    async def test_delete_note_tool_handler_exists(self, server):
        ...
        assert callable(getattr(server, 'handle_delete_note'))
    ...
    @pytest.mark.asyncio
    async def test_delete_note_tool_handler_validates_required_fields(self, server, mock_client):
...
class TestJoplinMCPServerListNotebooksTool:
    ...
    async def test_list_notebooks_tool_has_correct_schema(self, server):
        ...
        tools = server.get_available_tools()
        list_notebooks_tool = next(tool for tool in tools if tool.name == 'list_notebooks')
    ...
    async def test_list_notebooks_tool_handler_exists(self, server):
        ...
        assert callable(getattr(server, 'handle_list_notebooks'))
    
    @pytest.mark.asyncio
    async def test_list_notebooks_tool_handler_with_valid_data(self, server, mock_client):
...
class TestJoplinMCPServerPingJoplinTool:
    ...
    async def test_ping_joplin_tool_handler_exists(self, server):
        ...
        assert callable(getattr(server, 'handle_ping_joplin'))
    
    @pytest.mark.asyncio
    async def test_ping_joplin_tool_handler_with_successful_ping(self, server, mock_client):
...
class TestJoplinMCPServerGetNotebookTool:
    ...
    async def test_get_notebook_tool_handler_exists(self, server):
        ...
        assert callable(getattr(server, 'handle_get_notebook'))
    ...
    @pytest.mark.asyncio
    async def test_get_notebook_tool_handler_validates_required_fields(self, server, mock_client):
...
class TestJoplinMCPServerCreateNotebookTool:
    ...
    async def test_create_notebook_tool_has_correct_schema(self, server):
        ...
        tools = server.get_available_tools()
        create_notebook_tool = next(tool for tool in tools if tool.name == 'create_notebook')
    ...
    async def test_create_notebook_tool_handler_exists(self, server):
        ...
        assert callable(getattr(server, 'handle_create_notebook'))
    
    @pytest.mark.asyncio
    async def test_create_notebook_tool_handler_with_valid_data(self, server, mock_client):
    ...
    @pytest.mark.asyncio
    async def test_create_notebook_tool_handler_validates_required_fields(self, server, mock_client):
...
class TestJoplinMCPServerListTagsTool:
    ...
    async def test_list_tags_tool_handler_exists(self, server):
        ...
        assert callable(getattr(server, 'handle_list_tags'))
...
class TestJoplinMCPServerCreateTagTool:
    ...
    async def test_create_tag_tool_handler_exists(self, server):
        ...
        assert callable(getattr(server, 'handle_create_tag'))
    ...
    @pytest.mark.asyncio
    async def test_create_tag_tool_handler_validates_required_fields(self, server, mock_client):
...
class TestJoplinMCPServerTagNoteTool:
    ...
    async def test_tag_note_tool_handler_exists(self, server):
        ...
        assert callable(getattr(server, 'handle_tag_note'))
    ...
    @pytest.mark.asyncio
    async def test_tag_note_tool_handler_validates_required_fields(self, server, mock_client):
...
class TestJoplinMCPServerUntagNoteTool:
    ...
    async def test_untag_note_tool_handler_exists(self, server):
        ...
        assert callable(getattr(server, 'handle_untag_note'))
    ...
    @pytest.mark.asyncio
    async def test_untag_note_tool_handler_validates_required_fields(self, server, mock_client):
