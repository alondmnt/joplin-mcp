...
class TestJoplinMCPServerIntegration:
    ...
    async def test_server_initialization_and_capabilities(self, server):
        ...
        expected_tools = [
            ...
            'list_tags', 'create_tag', 'tag_note', 'untag_note', 'ping_joplin'
        ]
    ...
    async def test_tag_management_and_note_tagging_workflow(self, server, mock_client):
        ...
        tag_params = {
            'note_id': 'note_test_456',
            'tag_id': 'tag_integration_123'
        }
        ...
        assert 'Successfully removed tag from note' in untag_result['content'][0]['text']
    ...
    async def test_search_and_retrieval_workflow(self, server, mock_client):
        ...
        assert 'Found 2 note(s) for query: "integration test"' in search_result['content'][0]['text']
        ...
        for note_id in ['note_search_1', 'note_search_2']:
            ...
            assert f'Integration Test Note {note_id[-1]}' in get_result['content'][0]['text']
    ...
    async def test_connection_and_error_handling(self, server, mock_client):
        ...
        assert 'Joplin server connection successful' in ping_result['content'][0]['text']
